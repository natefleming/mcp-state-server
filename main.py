...
using the command: mcp-state-server
...
def main() -> None:
    ...
    Usage:
        Run with default port: uv run mcp-state-server
        Run with custom port: uv run mcp-state-server --port 8080
    """
    parser = argparse.ArgumentParser(description="Start the MCP State Server")
    parser.add_argument(
    ...
    uvicorn.run(
        "app:combined_app",  # Import path to the combined FastAPI application
