...
5. Initializing database connection and schema
...
mcp_server = FastMCP(name="mcp-state-server")
...
# Tools are defined in mcp_state_server/tools/
load_tools(mcp_server)

# Convert the MCP server to an HTTP application
...
async def serve_index() -> FileResponse | dict[str, str]:
    ...
    if STATIC_DIR.exists() and (STATIC_DIR / "index.html").exists():
        ...
        return {
            "message": "MCP State Server is running",
            "status": "healthy",
            "description": "MCP server for conversation and preference persistence",
        }
